...
if __name__ == '__main__':
    net_file_name = "state_dict_SVTDOS_300_300_300_0.4_0.3_0.3_41_10_1_1_0.0_fc_True_7500_4000_0.00025_cos-warm_5_2_single_vacuum_STO_600_cut9_gauss16_0.25_5_False_BN_seq_without_activate.pt"
    data_dir = "/public/WORK_backup/caizefeng/Datasets/vacuum_STO/single_vacuum_STO_600_cut9_gauss16_0.25_5"
    ldos_dir = "/public/WORK_backup/caizefeng/Datasets/vacuum_STO/single_vacuum_STO_600_cut9_gauss16_0.25_5"
    nets_dir = "/home/lzhpc/WORK/caizefeng/DeepDFT/nets"
    snapshot_label = "10001"
    level_name = "Vacuum"
    plot_type = "test"

    feature_file_path = os.path.join(data_dir, plot_type, snapshot_label) + '.npy'
    energy_file_path = os.path.join(ldos_dir, plot_type, "LDOS_" + snapshot_label) + '.txt'
    dos_file_path = os.path.join(ldos_dir, plot_type, "LDOS_" + snapshot_label) + '.npy'
    ...
    # feature = np.load(feature_file_path).astype(np.float32)
    feature = np.load(feature_file_path)
    # energy = np.loadtxt(energy_file_path).astype(np.float32)
    energy = np.loadtxt(energy_file_path)
    interval = np.mean(np.diff(energy))
    dos = np.load(dos_file_path)
    ...
    # sample = (torch.from_numpy(feature[:, :-1]) - train_mean.float()) / train_std.float()
    sample = (torch.from_numpy(feature[:, :-1]) - train_mean) / train_std
    with torch.no_grad():
        net.eval()
        dos_pred = (net(sample).sum(dim=0) / interval).numpy()

    dos_dft = dos.sum(axis=0) / interval

    # plot
    plt.figure(figsize=(10, 5))
    plt.plot(energy, dos_dft, label="DOS (DFT)", lw=1)
